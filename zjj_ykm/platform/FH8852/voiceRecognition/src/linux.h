...
static char *_itoa_s(int _Value, char * _DstBuf, int _Size, int _Radix)
{
	return itoa(_Value, _DstBuf, _Radix);
}

static char *___gcvt(double _val, int _numOfDigits, char *_result)
{
	sprintf(_result, "%f", _val);
	return _result;
}

static char *_gcvt_s(char *_buf, int _bufSize, double _Value, int _NumOfDigits)
{
