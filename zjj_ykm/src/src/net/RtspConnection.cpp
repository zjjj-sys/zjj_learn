...
RtspConnection::RtspConnection(RtspServer *rtspServer, int sockfd) : TcpConnection(rtspServer->envir(), sockfd),
...
void RtspConnection::handleReadBytes()
{
    ...
    if (mIsRtpOverTcp) // 处理TCP的client回复的心跳
    {
    ...
    return;
err:
    LOG_INFO("cancel rtsp_server fd\n");
    handleDisconnection();
}
...
bool RtspConnection::handleCmdDescribe()
{
    ...
    if (!session)
    {
        LOG_INFO("can't loop up %s session\n", mSuffix.c_str());
        return false;
    }
...
bool RtspConnection::handleCmdSetup()
{
    ...
    {
        /* 创建rtp over tcp */
        createRtpOverTcp(mTrackId, mSocket.fd(), mRtpChannel);      //创建一个RTP实例
        mRtpInstances[mTrackId]->setSessionId(mSessionId);
        session->addRtpInstance(mTrackId, mRtpInstances[mTrackId]);     //根据tarck_id 把RTP实例加入session 的track队列中
...
void RtspConnection::handleRtpOverTcp()
{
    ...
    uint16_t size;
    size = (buf[2] << 8) | buf[3];
    if (mInputBuffer.readableBytes() < size + 4) //有4个TCP头字节
    {
        return;
    }
